...
import { type ActionNode as ActionNodeType, type StatusNodeState, type StatusNode as StatusNodeType } from "@/types/nodes";
...
const statusNodeIconMap: IconMap<StatusNodeState> = {
    "unknown": <CircleQuestionMark size={16} className="stroke-gray-400 dark:stroke-gray-300" />,
...
export const StatusNode = memo(({ data, selected }: NodeProps<StatusNodeType>) => {
    ...
    return (
        <BaseNode selected={selected} className="px-3 py-2 max-w-md">
            <NodeHeader className="-mx-3 -mt-2 border-b">
                ...
                <NodeHeaderTitle>{data.title}</NodeHeaderTitle>
                <NodeHeaderActions>
                    <IconSelector selectedIcon={state}
